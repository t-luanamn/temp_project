...
{
    t_ray ray;
    float u = (2.0f * ((float)x + 0.5f) / (float)W_WIDTH - 1.0f)
              * mrt->camera.aspect_ratio * mrt->camera.scale;
    float v = (1.0f - 2.0f * ((float)y + 0.5f) / (float)W_HEIGHT) * mrt->camera.scale;

    ray.origin = mrt->camera.view_point;
    ray.direction = vector_normalise(vector_add(
        vector_add(vector_scale(basis.right, u), vector_scale(basis.up, v)), basis.forward));
